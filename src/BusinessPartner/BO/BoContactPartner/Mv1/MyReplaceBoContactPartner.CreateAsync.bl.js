...
me.setLastModifiedId(user.getPKey());
me.setCreatedById(user.getPKey());
me.setReportsToID(" ");
me.setActive(1);
me.setContactOwner(user.getPKey()); 
...
var promise = BoFactory.createListAsync(LO_CONTACTPARTNERADDRESS, {'params': []}).then(
  ...
  function (object) {
    ...
    return BoFactory.createListAsync('LuUser', me.getQueryBy("pKey", user.getPKey()));
  }).then(
  function (luLastModifiedBy) {
    me.setLuLastModifiedBy(luLastModifiedBy);
    return BoFactory.createListAsync('LuUser', Utils.getQueryBy("pKey", user.getPKey()));
  }).then(
  function (luCreatedBy) {
    me.setLuCreatedBy(luCreatedBy);
    return BoFactory.createListAsync('LuContact', Utils.getQueryBy("pKey", " "));
  }).then(
  function (luReportsTo) {
    me.setLuReportsTo(luReportsTo);
    return BoFactory.createListAsync('LuCustomer', Utils.getQueryBy("pKey", me.getAccount()));
  }).then(
  function (luAccount) {
    me.setLuAccount(luAccount);
    return BoFactory.createListAsync('LuUser', Utils.getQueryBy("pKey", me.getContactOwner()));
  }).then(
