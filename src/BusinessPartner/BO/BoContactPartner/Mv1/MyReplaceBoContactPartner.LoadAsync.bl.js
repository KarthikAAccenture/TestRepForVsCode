...
/*
.then(
  function (luLastModifiedBy) {
    me.setLuLastModifiedBy(luLastModifiedBy);
    return BoFactory.loadListAsync("LuMyUserLookup", me.getQueryBy("pKey", me.getLastModifiedId()));
  })*/
var promise = Facade.getObjectAsync(BO_CONTACTPARTNER, jsonQuery).then(
  ...
  function (loContactPartnerCustomerRelationJson) {
    ...
    //var loadLastModified = BoFactory.loadAsync('LuMyUserLookup', me.getQueryBy("pKey", me.getLastModifiedId()));
    me.setEmailEditable(isValidEmail);
    //me.setLuLastModifiedBy();
    me.setLoContactPartnerCustomerRelation(loContactPartnerCustomerRelationJson);
    return BoFactory.loadListAsync('LuUser', me.getQueryBy("pKey", me.getLastModifiedId()));
  }).then(
  function (luLastModifiedBy) {
    me.setLuLastModifiedBy(luLastModifiedBy);
    return BoFactory.loadListAsync('LuUser', Utils.getParamsForDeprecatedDS("pKey", me.getCreatedById()));
  }).then(
  function (luCreatedBy) {
    me.setLuCreatedBy(luCreatedBy);
    return BoFactory.loadListAsync('LuContact', Utils.getParamsForDeprecatedDS("pKey", me.getReportsToID()));
  }).then(
  function (luReportsTo) {
    me.setLuReportsTo(luReportsTo);
    return BoFactory.loadListAsync('LuCustomer', Utils.getParamsForDeprecatedDS("pKey", me.getAccount()));
  }).then(
  function (luAccount) {
    me.setLuAccount(luAccount);
    return BoFactory.loadListAsync('LuUser', Utils.getParamsForDeprecatedDS("pKey", me.getContactOwner()));
  }).then(
  function (luContactOwner) {
    me.setLuContactOwner(luContactOwner);
