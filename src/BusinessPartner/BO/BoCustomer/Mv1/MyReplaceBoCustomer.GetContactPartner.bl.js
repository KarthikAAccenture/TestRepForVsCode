 ...
 * @function replaceGetContactPartner
 * @this BoCustomer
 * @kind businessobject
 * @async
 * @namespace CUSTOM
...
//Check if NPS is available and takes the first NPS contact partner.

if (contactPartnerCount > 0) 
{
  for (var i = 0; i < contactPartnerCount; i++) 
  {
    if ( (ApplicationContext.get('user').salesOrg == Localization.resolve("Canada_SalesOrg") && contactPartners[i].nps === 1) || 
         (ApplicationContext.get('user').salesOrg == Localization.resolve("USA_SalesOrg") && contactPartners[i].pKey == me.getLuKeyContact().getPKey()) )
    {
      name = contactPartners[i].name;
      mainFunction = contactPartners[i].position;
      phone = contactPartners[i].phone1;
      mobile = contactPartners[i].phone2;
      contactPartnerPKey = contactPartners[i].getToPKey();
      email = contactPartners[i].email1;
      visible = true;
      break;
    }
  }
...
customerContactInfo.email = email;
customerContactInfo.mainFunction =  Utils.getToggleText("DomMyContactPosition", mainFunction);
